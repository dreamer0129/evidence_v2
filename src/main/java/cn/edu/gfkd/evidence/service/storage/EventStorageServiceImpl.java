 ...
 * 区块链事件存储服务实现
 * 
 * 主要职责： 1. 提供区块链事件的CRUD操作 2. 管理事件处理状态 3. 支持事务性操作和重试机制 4. 提供事件查询和统计功能
 */
@Service @Slf4j
public class EventStorageServiceImpl implements EventStorageService {
    ...
    public EventStorageServiceImpl(BlockchainEventRepository blockchainEventRepository,
    ...
    public BlockchainEvent saveEvent(BlockchainEvent event) {
        log.debug("Saving blockchain event: contract={}, eventType={}, blockNumber={}, txHash={}",
                event.getContractAddress(), event.getEventName(), event.getBlockNumber(),
                event.getTransactionHash());

        return retryHandler.executeWithRetrySync(() -> {
            // 检查事件是否已存在，避免重复处理
            boolean exists = blockchainEventRepository
                    .existsByContractAddressAndBlockNumberAndTransactionHashAndEventName(
                            event.getContractAddress(), event.getBlockNumber(),
            ...
            if (exists) {
                log.debug("Event already exists, skipping: txHash={}, eventType={}",
                        event.getTransactionHash(), event.getEventName());
                // 返回已存在的事件
                return blockchainEventRepository
                        .findByContractAddressAndBlockNumberAndTransactionHashAndEventName(
                                event.getContractAddress(), event.getBlockNumber(),
                                event.getTransactionHash(), event.getEventName())
                        .orElse(event);
            }
            ...
            log.info("Saved blockchain event with id: {} for evidenceId: {}", savedEvent.getId(),
                    event.getEventName());

            return savedEvent;
        }, "save blockchain event", dbLock);
    }
    ...
    @Override
    public Optional<BlockchainEvent> findByTransactionHashAndEventType(String transactionHash,
            String eventType) {
        log.debug("Finding events by transaction hash: {} and event type: {}", transactionHash,
                eventType);

        return retryHandler
                .executeWithRetry(
                        () -> blockchainEventRepository
                                .findByTransactionHashAndEventName(transactionHash, eventType),
    ...
    @Override
    public List<BlockchainEvent> findUnprocessedEventsByEventType(String eventType,
            Pageable pageable) {
        log.debug("Finding unprocessed events by type: {} with pagination: {}", eventType,
                pageable);

        return retryHandler
                .executeWithRetry(
                        () -> blockchainEventRepository
                                .findByEventNameAndIsProcessedFalse(eventType, pageable),
    ...
    public void markEventAsProcessed(Long eventId) {
        ...
        retryHandler.executeWithRetryTransactional(() -> {
            BlockchainEvent event = blockchainEventRepository.findById(eventId).orElseThrow(
                    () -> new BlockchainException("Event not found with id: " + eventId));
            ...
            log.info("Marked event as processed: id={}, transactionHash={}, eventType={}",
                    updatedEvent.getId(), updatedEvent.getTransactionHash(),
    ...
    public void markEventAsProcessed(String transactionHash, String eventType) {
        log.debug("Marking events as processed: transactionHash={}, eventType={}", transactionHash,
                eventType);
        ...
        return retryHandler
                .executeWithRetry(
                        () -> blockchainEventRepository.findById(eventId)
                                ...
                                .orElseThrow(() -> new BlockchainException(
    ...
    public long countUnprocessedEvents(String eventType) {
        ...
        return retryHandler
                .executeWithRetry(
                        () -> eventType != null
                                ? blockchainEventRepository
                                        .countByEventNameAndIsProcessedFalse(eventType)
    ...
    public int getEventProcessingFailures(Long eventId) {
        ...
        return retryHandler.executeWithRetry(
                () -> blockchainEventRepository.findById(eventId)
                        .map(BlockchainEvent::getProcessingFailures).orElse(0),
    ...
    public void incrementEventProcessingFailures(Long eventId) {
        ...
        retryHandler.executeWithRetryTransactional(() -> {
            ...
            int currentFailures = event.getProcessingFailures() != null
                    ? event.getProcessingFailures()
                    : 0;
    ...
    @Override
    public boolean eventExists(String contractAddress, BigInteger blockNumber,
            String transactionHash, String eventType) {
