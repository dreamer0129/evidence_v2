...
public class CertificateService {
    ...
    public String generateCertificate(EvidenceEntity evidence) throws CertificateGenerationException {
        ...
        try {
            ...
            if (!templateResource.exists()) {
                throw new CertificateGenerationException("Certificate template not found: static/proof_template.pdf");
            }
            ...
            log.error("Failed to generate certificate for evidence {}: {}", evidence.getEvidenceId(), e.getMessage(), e);
    ...
    private PDFont loadFont(PDDocument document) throws IOException {
        ...
        try {
            Resource fontResource = new ClassPathResource("static/fonts/simhei.ttf");
        ...
        try {
            return PDType0Font.load(document, new ClassPathResource("static/fonts/simhei.ttf").getInputStream());
        } catch (IOException e) {
            log.warn("Failed to load fallback font: {}", e.getMessage());
        }
        
        // Use standard Helvetica as last resort
        try {
            return PDType0Font.load(document, new java.io.File("src/main/resources/static/fonts/simhei.ttf"));
        } catch (IOException e) {
        ...
        for (PDField field : pdfForm.getFields()) {
            ...
            if (fieldContentMap.containsKey(fieldName)) {
                if (field instanceof PDTextField) {
                    ...
                    log.warn("Unsupported field type: {} for field: {}", field.getClass().getSimpleName(), fieldName);
                }
            }
        }
    }
