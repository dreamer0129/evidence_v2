    ...
    public void init() {
        ...
        storageMap = storageImplementations.stream()
            .collect(Collectors.toMap(
                CertificateStorage::getStorageType,
                Function.identity()
            ));
        ...
        if (defaultStorage == null) {
            log.warn("Configured storage type '{}' not found. Available types: {}", 
                    storageType, storageMap.keySet());
    ...
    public CertificateStorage getStorage(String type) {
        ...
        if (storage == null) {
            throw new IllegalArgumentException("Unknown storage type: " + type + 
                    ". Available types: " + storageMap.keySet());
        }
        return storage;
    }
