 ...
 * 证据撤销事件处理器
 * 
 * 主要职责： 1. 处理EvidenceRevoked事件 2. 将证据状态设置为已撤销 3. 记录撤销时间和撤销者信息 4. 确保撤销操作的准确性
 */
...
public class EvidenceRevokedProcessor implements BlockchainEventProcessor {
    ...
    public void processEvent(BlockchainEvent event) throws EventProcessingException {
        log.debug("Processing EvidenceRevoked event: id={}, txHash={}", event.getId(),
        ...
        try {
            ...
            log.info(
                    "Successfully revoked evidence for evidenceId: {} by revoker: {}, previous status: {}",
    ...
    public String getSupportedEventType() {
        return EvidenceEventType.EVIDENCE_REVOKED.name();
    }
